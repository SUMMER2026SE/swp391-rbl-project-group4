...
// Đợt đầu CHỈ hỗ trợ các dạng câu đơn không passage/không nghe (8 dạng cột language).
...
function assertImportableType(mondaiType) {
  if (!IMPORTABLE_TYPES.includes(mondaiType))
    throw httpError(400, 'Import file hiện chỉ hỗ trợ các dạng câu đơn (từ vựng/ngữ pháp), chưa hỗ trợ dạng đọc hiểu/nghe.');
}

// ── Template .xlsx phát hành cho admin ────────────────────────────────────────

function templateHeaders(n) {
  const headers = ['question_text'];
...
function buildImportTemplate(level, mondaiType) {
  ...
  const headers = templateHeaders(n);

  const rows = [headers];
  try {
    const ex = JSON.parse(meta.example);
    const row = [ex.question_text || ''];
  ...
  wsQuestions['!cols'] = headers.map(h =>
    ({ wch: h === 'question_text' ? 45 : h.startsWith('option_translation') ? 25 : h === 'explanation' || h === 'translation_vi' ? 35 : 18 }));
...
function requiredFields(n) {
  const req = ['question_text', 'correct'];
  for (let i = 1; i <= n; i++) req.push(`option_${i}`);
  return req;
}

// headers gốc của sheet → { mapping: {field: header gốc}, missing: [field bắt buộc chưa map được] }
function buildMapping(headers, n) {
  ...
  const missing = requiredFields(n).filter(f => mapping[f] === undefined);
  return { mapping, missing };
}

// 1 row đã map → { question } hoặc { error } hoặc null (dòng trống)
function rowToQuestion(row, mapping, meta) {
  ...
  if (!question_text) return { error: 'Thiếu nội dung câu hỏi (question_text).' };
...
{"question_text":"...","options":[đúng ${n} chuỗi],"correct_index":0-${n - 1},"explanation":"... hoặc null","translation_vi":"... hoặc null","option_translations":[${n} chuỗi] hoặc null}
- correct_index tính từ 0. Nếu dữ liệu ghi đáp án dạng 1-${n} hoặc A-${String.fromCharCode(64 + n)} thì quy đổi.
- explanation/translation_vi/option_translations bằng tiếng Việt; dữ liệu gốc thiếu thì để null (KHÔNG tự sáng tác).
...
async function parseJlptBankImport({ buffer, filename, mondaiType }) {
  ...
  // Câu ví dụ trong template — chặn nhập nhầm
  ...
  for (const { name, rows } of sheets) {
    ...
    const { mapping, missing } = buildMapping(headers, meta.options_count);
    ...
    rows.forEach((row, i) => {
      ...
      valid.push({ row: label, question: parsed.question });
    });
  }
  ...
  if (valid.length === 0 && errors.length === 0 && unmappedSheets.length > 0) {
    ...
    items.forEach((it, i) => {
      const question = {
        question_text:       typeof it.question_text === 'string' ? it.question_text.trim() : '',
      ...
      const msg = !question.question_text ? 'Thiếu nội dung câu hỏi.' : validateQuestionPayload(question);
