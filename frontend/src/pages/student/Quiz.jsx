...
export default function Quiz({ Layout = StudentLayout }) {
  ...
  // Chỉ bắt đầu sau khi vào chế độ toàn màn hình nghiêm ngặt
  const examStarted = !isStrictFs || lockdown.active;
  ...
  useEffect(() => {
    if (!examStarted || result || !isStrictFs) return;
  ...
  if (result) {
    ...
    // Quiz có cấu hình ngưỡng → dùng nhãn đạt/không đạt từ backend; nếu không, giữ hiển thị định tính cũ.
    const hasThreshold = result.passed === true || result.passed === false;
    const good = hasThreshold ? result.passed : pctScore >= 70;
    const caption = hasThreshold
        ...
        <div className="max-w-md mx-auto text-center">
          <div className="glass-card rounded-2xl p-10">
            <span className={`material-symbols-outlined text-7xl mb-4 block ${good ? 'text-green-500' : 'text-tsubaki-red'}`}>
              {good ? 'emoji_events' : 'sentiment_dissatisfied'}
            </span>
            ...
            <p className="text-on-muted mb-6">{pctScore}% — {caption}</p>
