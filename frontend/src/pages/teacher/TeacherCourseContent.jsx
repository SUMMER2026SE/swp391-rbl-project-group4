  ...
  return (
    <TeacherLayout title="Nội dung khóa học">
      ...
      <div className="space-y-4">
          ...
          <div key={unit.id} className="bg-white border border-outline/30 shadow-sm rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-outline/10 bg-surface-container-lowest/60">
              <div className="flex items-center gap-3 min-w-0">
                <span className="w-7 h-7 rounded-full bg-tsubaki-red/10 text-tsubaki-red text-xs font-bold flex items-center justify-center shrink-0">{idx + 1}</span>
                <div className="min-w-0">
                  <h3 className="font-semibold text-on-surface text-sm truncate">{unit.title}</h3>
                  <p className="text-xs text-on-muted">{(unit.lessons || []).length} mục</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button onClick={() => openEditUnit(unit)} className="p-1.5 text-on-muted hover:text-tsubaki-red hover:bg-tsubaki-red/10 rounded-lg transition-colors"><span className="material-symbols-outlined text-lg">edit</span></button>
            ...
            <div className="p-4 space-y-2">
              {(unit.lessons || []).map(item => {
                ...
                return (
                  <div key={item.id} className="flex items-center justify-between px-3 py-2.5 rounded-xl border border-outline/20 hover:border-tsubaki-red/30 transition-all group/item">
                    <button onClick={() => openDeepEditor(item)} className="flex items-center gap-3 min-w-0 flex-1 text-left" title="Soạn nội dung">
                      ...
                      <span className="font-medium text-sm text-on-surface truncate group-hover/item:text-tsubaki-red transition-colors">{item.title}</span>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="text-xs text-on-muted hidden sm:block mr-1">{meta.label}</span>
                      <button onClick={() => openEditInfo(item)} title="Sửa thông tin" className="p-1 text-on-muted hover:text-tsubaki-red rounded-lg transition-colors"><span className="material-symbols-outlined text-[18px]">tune</span></button>
              ...
              <button onClick={() => openAddItem(unit)} className="w-full py-3 border-2 border-dashed border-outline/25 rounded-xl text-on-muted text-sm font-medium hover:border-tsubaki-red/40 hover:text-tsubaki-red transition-all flex items-center justify-center gap-2">
        ...
        <button onClick={openAddUnit} className="w-full py-10 bg-white border-2 border-dashed border-outline/25 rounded-2xl flex flex-col items-center justify-center gap-2 hover:border-tsubaki-red/50 hover:bg-surface-container-low/30 transition-all group">
          <div className="w-11 h-11 rounded-full bg-surface-container-highest/50 group-hover:bg-tsubaki-red group-hover:text-white transition-all flex items-center justify-center"><span className="material-symbols-outlined text-2xl">add</span></div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {LESSON_TYPES.map(t => (
                <button key={t.value} type="button"
                  ...
                  className={`flex flex-col items-center gap-1 p-3 rounded-xl border-2 transition-all text-sm font-medium ${itemForm.lesson_type === t.value ? 'border-tsubaki-red bg-tsubaki-red/5 text-tsubaki-red' : 'border-outline/30 hover:border-tsubaki-red/40 text-on-muted'}`}>
                  <span className={`material-symbols-outlined text-2xl ${itemForm.lesson_type === t.value ? 'text-tsubaki-red' : t.color}`}>{t.icon}</span>
                  {t.label}
                </button>
              ))}
            </div>
