...
function formFromRow(row) {
	const base = {
		...
		listening_passage_id: row.listening_passage_id || "",
	};
	switch (base.question_type) {
		case "single_choice":
			return {
				...
				correct_answer_single: row.correct_answer || "",
			};
		...
		case "matching":
			return {
				...
				matching_pairs:
					...
					:	EMPTY_FORM.matching_pairs,
			};
		case "ordering":
			return {
				...base,
				ordering_items:
					Array.isArray(row.correct_answer) && row.correct_answer.length ?
						[...row.correct_answer, "", ""]
					:	EMPTY_FORM.ordering_items,
			};
		case "fill_blank": {
...
function buildPayload(form) {
	const base = {
		...
		status: "approved",
		passage_id: form.passage_id || null,
		listening_passage_id: form.listening_passage_id || null,
	};
	switch (form.question_type) {
		case "single_choice":
			return {
				...
				correct_answer: form.correct_answer_single,
			};
		case "multiple_choice":
			return {
				...base,
				options: form.options.filter(Boolean),
				correct_answer: form.correct_answers_multi,
			};
		case "matching": {
		...
		case "fill_blank": {
			const ans = form.fill_blank_answers
				.split(",")
				.map((s) => s.trim())
				.filter(Boolean);
...
function TypeBadge({ type, sm }) {
	...
	return (
		<span
			className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-semibold ${cfg.color} ${sm ? "text-[10px]" : "text-xs"}`}
		>
			<span className={`material-symbols-outlined ${sm ? "text-[11px]" : "text-[13px]"}`}>
				{cfg.icon}
			</span>
			{cfg.label}
		</span>
		...
		<div className="bg-amber-50 border border-amber-200 rounded-xl overflow-hidden mb-4">
			<div className="flex items-center justify-between px-4 py-2.5 bg-amber-100/60 border-b border-amber-200">
				<div className="flex items-center gap-2 flex-wrap">
					...
					{passage.title && (
						<span className="text-xs font-semibold text-amber-700">— {passage.title}</span>
					)}
					{hasImage && !hasText && (
					...
					{hasImage && (
						<div className="px-4 pt-3">
							<img
								...
								alt="passage"
								className="w-full rounded-lg max-h-48 object-contain bg-white border border-amber-200"
							/>
						</div>
					)}
					{hasText && (
		...
		<div className="bg-sky-50 border border-sky-200 rounded-xl overflow-hidden mb-4">
			<div className="flex items-center justify-between px-4 py-2.5 bg-sky-100/60 border-b border-sky-200">
				<div className="flex items-center gap-2 flex-wrap">
					...
					{passage.title && (
						<span className="text-xs font-semibold text-sky-700">— {passage.title}</span>
					)}
...
function PreviewModal({ item, onClose }) {
                ...
                ${opt === item.correct_answer ? "border-emerald-400 bg-emerald-50 text-emerald-800 font-semibold" : "border-outline bg-surface-low text-charcoal"}`}
                  ...
                  ${opt === item.correct_answer ? "bg-emerald-500 text-white" : "bg-outline/30 text-on-muted"}`}
									...
									{opt === item.correct_answer && (
										<span className="material-symbols-outlined text-[16px] ml-auto text-emerald-500 shrink-0">
											check_circle
										</span>
									)}
								</div>
			...
			case "multiple_choice":
				return Array.isArray(item.options) && item.options.length ?
						<div className="space-y-2">
							{item.options.map((opt, i) => {
								const correct =
									Array.isArray(item.correct_answer) && item.correct_answer.includes(opt);
								return (
									<div
										key={i}
										className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border text-sm
						...
						<div className="space-y-2">
							...
							{item.options.map((p, i) => (
								...
								>
									...
									<span className="material-symbols-outlined text-on-muted text-lg">
										arrow_forward
									</span>
				...
				return Array.isArray(item.correct_answer) && item.correct_answer.length ?
						<div className="space-y-2">
							...
							{item.correct_answer.map((it, i) => (
								<div
									key={i}
									className="flex items-center gap-3 px-4 py-2.5 bg-emerald-50 border border-emerald-300 rounded-xl text-sm font-semibold text-emerald-800"
								>
	...
	return (
		...
		>
			...
			<div
				className="relative bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden max-h-[92vh] flex flex-col"
				...
				<div className="p-6 overflow-y-auto">
					...
					<div className="flex items-start justify-between gap-3 mb-4">
						<div className="flex flex-wrap gap-2">
							...
							{item.level && (
								<span
									className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${LEVEL_COLORS[item.level]}`}
								>
									{item.level}
								</span>
							)}
							...
							{item.difficulty && (
								<span
									className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${DIFF_COLORS[item.difficulty]}`}
								>
...
function QuestionForm({ form, setForm, passages, listeningPassages }) {
	...
	const selectedListeningPassage =
		listeningPassages?.find((p) => p.id === form.listening_passage_id) || null;
	...
	const setOpt = (i, v) => {
		...
		if (typ === "single_choice" && form.correct_answer_single === form.options[i])
			next.correct_answer_single = "";
		setForm(next);
	};
	const toggleMulti = (opt) => {
		const arr =
			form.correct_answers_multi.includes(opt) ?
	...
	const setPair = (i, side, v) =>
		setForm({
			...form,
			matching_pairs: form.matching_pairs.map((p, idx) => (idx === i ? { ...p, [side]: v } : p)),
		});
	const addPair = () =>
		setForm({ ...form, matching_pairs: [...form.matching_pairs, { left: "", right: "" }] });
	const removePair = (i) =>
		setForm({ ...form, matching_pairs: form.matching_pairs.filter((_, idx) => idx !== i) });
	const setOrdItem = (i, v) =>
		setForm({ ...form, ordering_items: form.ordering_items.map((x, idx) => (idx === i ? v : x)) });
	const addOrdItem = () => setForm({ ...form, ordering_items: [...form.ordering_items, ""] });
	const removeOrdItem = (i) =>
		setForm({ ...form, ordering_items: form.ordering_items.filter((_, idx) => idx !== i) });
		...
		<div className="space-y-5">
			...
			<div>
						...
						<button
							...
							onClick={() =>
								setForm({
									...
									listening_passage_id: form.listening_passage_id,
								})
							}
                ...
                ${form.question_type === qt.value ? "border-tsubaki-red bg-tsubaki-red/5 text-tsubaki-red" : "border-outline text-on-muted hover:border-tsubaki-red/50"}`}
						>
							<span
								className={`material-symbols-outlined text-[18px] shrink-0 ${form.question_type === qt.value ? "text-tsubaki-red" : ""}`}
							>
								{qt.icon}
							</span>
			...
			<div>
				...
				<select
					...
					onChange={(e) =>
						setForm({ ...form, passage_id: e.target.value, listening_passage_id: "" })
					}
				...
				>
					...
					{passages.map((p) => (
						...
						>
							...
							{p.level ? ` [${p.level}]` : ""} — {p.question_count} câu hỏi
						</option>
					))}
				</select>
				{selectedPassage && (
			...
			<div>
				...
				<select
					value={form.listening_passage_id}
					onChange={(e) =>
						setForm({ ...form, listening_passage_id: e.target.value, passage_id: "" })
					}
					...
					<div className="mt-2 bg-sky-50 border border-sky-200 rounded-xl p-3 flex items-center gap-3">
						...
						<div className="min-w-0">
							<p className="text-xs font-bold text-sky-800 truncate">
								{selectedListeningPassage.title || "Bài nghe"}
							</p>
							{selectedListeningPassage.description && (
			...
			<div>
				...
				{typ === "fill_blank" && (
					<p className="text-xs text-on-muted mb-1">
						Ví dụ:{" "}
						<span className="font-mono bg-surface-low px-1 rounded">私は___に住んでいます</span>
					</p>
				)}
				<textarea
			...
			{typ === "single_choice" && (
				<>
					...
					<div>
						...
						<select
							value={form.correct_answer_single}
							onChange={(e) => setForm({ ...form, correct_answer_single: e.target.value })}
			...
			{typ === "multiple_choice" && (
				<>
					<div>
						...
						{form.options.map((opt, i) => (
							<div
								...
								className="flex items-center gap-2 mb-2"
							>
								<span className="w-7 h-7 shrink-0 rounded text-xs font-bold bg-surface-low border border-outline text-on-muted flex items-center justify-center">
									{String.fromCharCode(65 + i)}
								</span>
								<input
									value={opt}
					...
					<div>
						...
						<div className="space-y-2">
							{form.options.filter(Boolean).length === 0 && (
								<p className="text-xs text-on-muted italic">Nhập các lựa chọn trước.</p>
							)}
							{form.options.filter(Boolean).map((opt, i) => {
                  ...
                  ${checked ? "border-emerald-400 bg-emerald-50" : "border-outline hover:border-emerald-300"}`}
									>
										...
										<span className="text-sm font-medium">
											{String.fromCharCode(65 + form.options.indexOf(opt))}. {opt}
										</span>
										{checked && (
											<span className="ml-auto text-xs font-bold text-emerald-600">✓ Đúng</span>
										)}
									</label>
								);
							})}
				...
				<div>
					<div className="flex items-center justify-between mb-2">
						...
						<button
							type="button"
							onClick={addPair}
							className="flex items-center gap-1 text-xs text-tsubaki-red font-semibold hover:underline"
						>
					...
					{form.matching_pairs.map((p, i) => (
						<div
							key={i}
							className="grid grid-cols-12 gap-1 mb-2 items-center"
						>
							<input
								value={p.left}
				...
				<div>
					<div className="flex items-center justify-between mb-1">
						<div>
							<label className="text-sm font-medium text-on-muted">
								Các phần tử (theo thứ tự đúng)
							</label>
							<p className="text-xs text-on-muted mt-0.5">
								Học viên sẽ thấy bị xáo trộn và cần sắp xếp lại.
							</p>
						</div>
						<button
							type="button"
							onClick={addOrdItem}
							className="flex items-center gap-1 text-xs text-tsubaki-red font-semibold hover:underline ml-2 shrink-0"
						>
					...
					<div className="mt-2 space-y-2">
						{form.ordering_items.map((item, i) => (
							...
							>
								<span className="w-7 h-7 shrink-0 rounded-full bg-tsubaki-red/10 text-tsubaki-red text-xs font-bold flex items-center justify-center">
									{i + 1}
								</span>
								<input
									value={item}
									onChange={(e) => setOrdItem(i, e.target.value)}
									placeholder={`Phần tử ${i + 1}`}
									className="flex-1 px-3 py-2 border border-outline rounded-xl text-sm outline-none focus:border-tsubaki-red transition-colors"
								/>
								<div className="flex gap-1">
									{[
										[-1, "keyboard_arrow_up", i === 0],
										[1, "keyboard_arrow_down", i === form.ordering_items.length - 1],
									].map(([dir, icon, dis]) => (
										<button
											key={icon}
											type="button"
											onClick={() => moveOrdItem(i, dir)}
											disabled={dis}
									...
									<button
										type="button"
										onClick={() => removeOrdItem(i)}
										disabled={form.ordering_items.length <= 2}
			...
			{typ === "fill_blank" && (
				<div>
					<label className="block text-sm font-medium text-on-muted mb-1">
						Đáp án chấp nhận <span className="font-normal text-xs">(phân cách bằng dấu phẩy)</span>
					</label>
					<input
						value={form.fill_blank_answers}
						onChange={(e) => setForm({ ...form, fill_blank_answers: e.target.value })}
			...
			{typ === "short_answer" && (
				<div>
					...
					<p className="text-xs text-on-muted mb-1">
						Để giáo viên tham khảo khi chấm điểm thủ công.
					</p>
					<textarea
						value={form.correct_answer_short}
						onChange={(e) => setForm({ ...form, correct_answer_short: e.target.value })}
						rows={3}
			...
			<div>
				...
				<textarea
					...
					rows={2}
					placeholder="Giải thích thêm về đáp án..."
					className="w-full px-4 py-3 bg-white border border-outline rounded-xl text-sm outline-none focus:border-tsubaki-red resize-none transition-colors"
				/>
			</div>
		</div>
...
function PassagePreviewSheet({ passage, questions, onClose }) {
	...
	const QNum = ({ n }) => (
		<span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-tsubaki-red text-white text-xs font-bold mr-2 shrink-0">
			{n}
		</span>
	);
	...
	const renderQuestion = (q, qi) => {
		...
		if (q.question_type === "single_choice") {
			...
			return (
				<div>
					<p className="font-medium text-sm mb-3 flex items-start">
						<QNum n={qi + 1} />
						{q.question_text}
					</p>
					<div className="space-y-2">
						{opts.map((opt, oi) => {
							...
							return (
								<button
									key={oi}
									onClick={() => setAnswer(qi, "single_choice", opt)}
									disabled={submitted}
								...
								>
									<span
										className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold shrink-0 ${
											...
											: sel ? "border-tsubaki-red bg-tsubaki-red text-white"
											: "border-outline/50 text-on-muted"
										}`}
									>
										{correct ?
											"✓"
		...
		if (q.question_type === "multiple_choice") {
			...
			return (
				<div>
					<p className="font-medium text-sm mb-1 flex items-start">
						<QNum n={qi + 1} />
						{q.question_text}
					</p>
					...
					<div className="space-y-2">
						{opts.map((opt, oi) => {
							...
							return (
								<button
									key={oi}
									onClick={() => setAnswer(qi, "multiple_choice", opt)}
									disabled={submitted}
									className={`w-full text-left flex items-center gap-3 px-4 py-2.5 rounded-xl border text-sm transition-all ${
								...
								>
									<span
										className={`w-5 h-5 rounded border-2 flex items-center justify-center text-xs font-bold shrink-0 ${
											...
											: isSelected ? "border-tsubaki-red bg-tsubaki-red text-white"
											: "border-outline/50"
										}`}
									>
										{isCorrect ?
		...
		if (q.question_type === "matching") {
			...
			return (
				<div>
					...
					<div className="space-y-2">
						{pairs.map((pair, pi) => (
							...
							>
								...
								<span className="material-symbols-outlined text-on-muted text-[18px]">
									compare_arrows
								</span>
								<div
									className={`px-3 py-2 rounded-xl text-sm border ${submitted ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-dashed border-outline/50 text-on-muted italic text-xs"}`}
								>
					...
					{!submitted && (
						<p className="text-xs text-on-muted mt-2">Đáp án sẽ hiện sau khi nộp bài.</p>
					)}
					{explanation}
				</div>
			);
				...
				<div>
					...
					<div className="space-y-2">
						{items.map((item, ii) => (
							<div
								key={ii}
								className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm ${submitted ? "border-emerald-300 bg-emerald-50" : "border-outline/40 bg-surface-low"}`}
							>
					...
					{!submitted && (
						<p className="text-xs text-on-muted mt-2">Thứ tự đúng sẽ hiện sau khi nộp bài.</p>
					)}
					{explanation}
				</div>
			);
		...
		return (
			<div className="glass-card rounded-xl p-4">
				<span
					className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${qt?.color || "bg-surface"}`}
				>
	...
	return (
		<div className="fixed inset-0 z-[70] bg-gray-50 flex flex-col overflow-hidden">
			...
			<div className="h-14 flex items-center px-5 border-b border-outline/30 gap-3 bg-white shrink-0">
				...
				<span className="px-2.5 py-0.5 text-xs font-bold bg-sumire-purple/10 text-sumire-purple rounded-full">
					Xem trước
				</span>
				...
				<div className="w-1/2 border-r border-outline/30 overflow-y-auto p-6 bg-white">
					...
					{passage.image_url && (
						<img
							src={passage.image_url}
							alt={passage.title || ""}
							className="w-full rounded-xl mb-5 object-contain max-h-64 bg-surface-low border border-outline"
						/>
					)}
					{passage.content ?
						<p
					...
					:	!passage.image_url && (
							<p className="text-sm text-on-muted italic">Chưa có nội dung bài đọc.</p>
						)
					}
				</div>
				...
				<div className="flex-1 overflow-y-auto p-6">
					{questions.length === 0 ?
						<div className="text-center py-20">
							<span className="material-symbols-outlined text-5xl text-on-muted/20 block mb-3">
								help
							</span>
					...
					:	<div className="space-y-7">
							{questions.map((q, qi) => (
								<div
									key={q.id || qi}
									className="bg-white rounded-2xl border border-outline/40 shadow-sm p-5"
								>
									{renderQuestion(q, qi)}
								</div>
							))}
...
function PassageEditorSheet({
		...
		initialPassage ?
			{
				...
				content: initialPassage.content || "",
				image_url: initialPassage.image_url || "",
				level: initialPassage.level || "",
				topic: initialPassage.topic || "",
				source: initialPassage.source || "",
			}
		:	{ ...EMPTY_PASSAGE },
	);
	...
	const loadQuestions = useCallback(
		async (pid) => {
			...
			try {
				...
				setAlert({ type: "error", msg: "Không thể tải câu hỏi." });
			} finally {
				setQLoading(false);
			}
		},
	...
	useEffect(() => {
		if (currentId) loadQuestions(currentId);
	}, [currentId, loadQuestions]);

	const handleImageSelect = async (file) => {
		if (!file) return;
		if (!file.type.startsWith("image/"))
			return setAlert({ type: "error", msg: "Chỉ chấp nhận file hình ảnh." });
		if (file.size > 5 * 1024 * 1024) return setAlert({ type: "error", msg: "File tối đa 5MB." });
		setUploading(true);
		try {
			const fd = new FormData();
			fd.append("image", file);
			const r = await api.post(`${apiBase}/reading-passages/upload`, fd, {
				headers: { "Content-Type": "multipart/form-data" },
			});
			setForm((prev) => ({ ...prev, image_url: r.data.url }));
		} catch {
			setAlert({ type: "error", msg: "Không thể tải ảnh lên." });
		} finally {
			setUploading(false);
	...
	const handleSavePassage = async () => {
		if (!form.content.trim() && !form.image_url)
			return setAlert({ type: "error", msg: "Bài đọc cần có nội dung text hoặc hình ảnh." });
		setSaving(true);
		try {
			const payload = {
				title: form.title,
			...
			setAlert({
				type: "success",
				msg: currentId ? "Đã cập nhật bài đọc." : "Đã thêm bài đọc mới.",
			});
			onSaved();
		} catch (e) {
	...
	const handleSaveQuestion = async () => {
		if (!qForm.question_text.trim())
			return setAlert({ type: "error", msg: "Chưa nhập nội dung câu hỏi." });
		setQSaving(true);
		try {
	...
	const handleUnlinkQuestion = async (q) => {
		if (!(await confirm(`Bỏ liên kết câu hỏi này khỏi bài đọc? Câu hỏi vẫn còn trong ngân hàng.`)))
			return;
		try {
	...
	const openImportModal = async () => {
		...
		try {
			...
			setAlert({ type: "error", msg: "Không thể tải ngân hàng câu hỏi." });
		} finally {
	...
	const handleImportSelected = async () => {
		...
		try {
			await Promise.all(
				ids.map((id) => api.put(`${apiBase}/question-bank/${id}`, { passage_id: currentId })),
			);
			...
			setAlert({ type: "error", msg: "Không thể liên kết câu hỏi." });
		}
	};

	const passageForForm =
		currentId ?
			[
				{
					id: currentId,
					title: form.title || "(Bài đọc hiện tại)",
			...
			<div className="fixed inset-0 z-50 bg-white flex flex-col overflow-hidden">
				...
				<div className="h-14 flex items-center px-4 border-b border-outline/30 gap-3 bg-white shrink-0">
					...
					<div className="flex-1 min-w-0">
						<h2 className="font-bold text-sm truncate">
							{form.title || (currentId ? "Bài đọc" : "Bài đọc mới")}
						</h2>
						{currentId && <p className="text-xs text-on-muted">{questions.length} câu hỏi</p>}
					</div>
					<button
						onClick={() => setPreview(true)}
						disabled={!currentId && !form.content.trim() && !form.image_url}
				...
				<div className="flex-1 flex overflow-hidden">
					...
					<div className="w-2/5 border-r border-outline/30 overflow-y-auto p-5 space-y-4 shrink-0">
						<p className="text-xs font-bold text-on-muted uppercase tracking-wide">
							Nội dung bài đọc
						</p>
								...
								<div className="relative rounded-xl overflow-hidden border border-outline bg-surface-low">
									...
									<div className="absolute top-2 right-2 flex gap-1">
										<button
											...
											className="px-2.5 py-1 bg-white/90 backdrop-blur-sm rounded-lg text-xs font-semibold border border-outline/40 shadow-sm"
										>
											Đổi ảnh
										</button>
										<button
                  ...
                  ${uploading ? "border-tsubaki-red/50 bg-tsubaki-red/5" : "border-outline hover:border-tsubaki-red hover:bg-surface-low"}`}
								>
									{uploading ?
										<div className="flex flex-col items-center gap-1.5">
											<div className="w-6 h-6 border-2 border-tsubaki-red border-t-transparent rounded-full animate-spin" />
											<p className="text-xs text-tsubaki-red font-semibold">Đang tải...</p>
										</div>
									:	<>
											<span className="material-symbols-outlined text-3xl text-on-muted/40 block mb-1">
												add_photo_alternate
											</span>
											<p className="text-xs font-semibold text-charcoal">Nhấn hoặc kéo thả ảnh</p>
											<p className="text-xs text-on-muted">JPG, PNG — tối đa 5MB</p>
										</>
									}
								</div>
							}
						...
						<div>
							...
							<textarea
								...
								rows={10}
								placeholder="Nhập nội dung bài đọc tiếng Nhật..."
						...
						<Input
							...
							placeholder="JLPT N3 2023, Minna no Nihongo..."
						/>
					</div>
					...
					<div className="flex-1 overflow-y-auto p-5">
						...
						: questions.length === 0 ?
							<div className="text-center py-16">
								<span className="material-symbols-outlined text-4xl text-on-muted/20 block mb-2">
									help
								</span>
								...
								{currentId && (
									<button
										onClick={openAddQuestion}
										className="mt-2 text-sm font-semibold text-tsubaki-red hover:underline"
									>
						...
						:	<div className="space-y-3">
								{questions.map((q, qi) => {
									...
									return (
										<div
											key={q.id}
											className="glass-card rounded-xl p-4 group hover:shadow-sm transition-shadow"
										>
											<div className="flex items-start gap-3">
												<span
													className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold shrink-0 mt-0.5 ${qt?.color || "bg-surface text-on-muted"}`}
												>
													<span className="material-symbols-outlined text-[13px]">{qt?.icon}</span>
													{qt?.label || q.question_type}
												</span>
												<div className="flex-1 min-w-0">
													<p className="text-sm font-medium text-charcoal line-clamp-2">
														<span className="text-tsubaki-red font-bold mr-1">{qi + 1}.</span>
														{q.question_text}
													</p>
													{Array.isArray(q.options) && q.options.length > 0 && (
														<div className="mt-1.5 space-y-0.5">
															{q.options.slice(0, 3).map((o, oi) => (
																<p
																	key={oi}
																	className="text-xs text-on-muted truncate"
																>
																	{String.fromCharCode(65 + oi)}.{" "}
																	{typeof o === "string" ? o : o.option_text || o.left || ""}
																</p>
															))}
															{q.options.length > 3 && (
																<p className="text-xs text-on-muted/60">
																	+{q.options.length - 3} đáp án khác
																</p>
															)}
														</div>
													)}
												...
												<div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
													...
													<button
														onClick={() => handleUnlinkQuestion(q)}
														title="Bỏ liên kết khỏi bài đọc này"
					...
					>
						<div className="flex items-center justify-between px-6 py-4 border-b border-outline/30 shrink-0">
							<div>
								...
								<p className="text-xs text-on-muted mt-0.5">
									Chọn câu hỏi để liên kết vào bài đọc này
								</p>
							</div>
							<button
								onClick={() => setImportModal(false)}
								className="w-8 h-8 rounded-full hover:bg-surface-low flex items-center justify-center text-on-muted"
							>
								<span className="material-symbols-outlined text-lg">close</span>
							</button>
						</div>
						<div className="overflow-y-auto flex-1 p-4 space-y-2">
							{bankLoading ?
								<div className="flex justify-center py-12">
									<div className="w-7 h-7 border-2 border-tsubaki-red border-t-transparent rounded-full animate-spin" />
								</div>
							: bankItems.length === 0 ?
								<p className="text-sm text-on-muted text-center py-12">
									Không có câu hỏi nào khác trong ngân hàng.
								</p>
							:	bankItems.map((q) => {
									const qt = TYPE_MAP[q.question_type];
                    ...
                    ${checked ? "border-tsubaki-red bg-tsubaki-red/5" : "border-outline/30 hover:bg-surface-low"}`}
										>
											<input
												...
												checked={checked}
												onChange={() => {}}
												className="mt-0.5 accent-tsubaki-red shrink-0"
											/>
											<div className="flex-1 min-w-0">
												...
												<div className="flex items-center gap-1.5 mt-1">
													{qt && (
														<span
															className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold ${qt.color}`}
														>
													...
													{q.reading_passages && (
														<span className="text-[10px] px-1.5 py-0.5 bg-amber-50 border border-amber-200 rounded font-semibold text-amber-700">
															{q.reading_passages.title || "Bài đọc"}
														</span>
													)}
													{!q.passage_id && !q.listening_passage_id && (
														<span className="text-[10px] px-1.5 py-0.5 bg-surface-low border border-outline/40 rounded font-semibold text-on-muted">
															Độc lập
														</span>
													)}
												</div>
						...
						<div className="flex items-center justify-between px-6 py-4 border-t border-outline/20 shrink-0">
							<p className="text-sm text-on-muted">
								Đã chọn: <strong className="text-charcoal">{selectedImportIds.size}</strong> câu hỏi
							</p>
							<div className="flex gap-2">
								...
								<Button
									...
									disabled={!selectedImportIds.size}
								>
...
function PassagesTab({ passages, onRefresh, setAlert, apiBase = "/admin" }) {
	...
	const handleDelete = async (p) => {
		if (
			!(await confirm(
				`Xóa bài đọc "${p.title || "này"}"? Các câu hỏi liên kết sẽ bị hủy liên kết.`,
			))
		)
			return;
		try {
			await api.delete(`${apiBase}/reading-passages/${p.id}`);
		...
		<>
			...
			:	<div className="grid gap-4">
					{passages.map((p) => (
						<div
							key={p.id}
							className="glass-card rounded-2xl overflow-hidden group hover:shadow-md transition-shadow"
						>
							{p.image_url && (
									...
									<div className="flex-1 min-w-0">
										<div className="flex items-center gap-2 mb-1 flex-wrap">
											<h3 className="font-bold text-charcoal">
												{p.title || (
													<span className="text-on-muted italic font-normal">Không có tiêu đề</span>
												)}
											</h3>
											{p.level && (
												<span
													className={`px-2 py-0.5 rounded-full text-xs font-bold ${LEVEL_COLORS[p.level]}`}
												>
													{p.level}
												</span>
											)}
											{p.topic && (
												<span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-surface-low border border-outline text-on-muted">
													{p.topic}
												</span>
											)}
											{p.image_url && !p.content && (
												<span className="flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded font-bold">
													<span className="material-symbols-outlined text-[11px]">image</span>Hình
													ảnh
												</span>
											)}
										...
										{p.content ?
											<p className="text-sm text-on-muted line-clamp-2 leading-relaxed">
												{p.content}
											</p>
										:	!p.image_url && (
												<p className="text-sm text-on-muted italic">Không có nội dung</p>
											)
										}
									</div>
									<div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
										<button
											onClick={() => setViewPassage(p)}
											title="Xem nội dung"
										...
										<button
											onClick={() => setEditorPassage({ passage: p })}
											title="Sửa"
											className="p-2 rounded-lg text-on-muted hover:bg-surface-low hover:text-charcoal transition-colors"
										>
							...
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<div className="flex flex-wrap gap-2 mt-0.5">
										{viewPassage.level && (
											<span
												className={`px-2 py-0.5 rounded-full text-xs font-bold ${LEVEL_COLORS[viewPassage.level]}`}
											>
												{viewPassage.level}
											</span>
										)}
										{viewPassage.source && (
											<span className="text-xs text-on-muted">Nguồn: {viewPassage.source}</span>
										)}
										<span className="text-xs text-on-muted">
											{viewPassage.question_count} câu hỏi
										</span>
									</div>
								</div>
							</div>
						...
						<div className="overflow-y-auto">
							{viewPassage.image_url && (
								<div className="px-6 pt-6">
									<img
										src={viewPassage.image_url}
										alt={viewPassage.title || "bài đọc"}
										className="w-full rounded-xl max-h-72 object-contain bg-surface-low border border-outline"
									/>
								</div>
							)}
							{viewPassage.content && (
								<div className="p-6">
									<p className="text-sm text-charcoal leading-loose whitespace-pre-wrap">
										{viewPassage.content}
									</p>
								</div>
							)}
...
function ListeningPassagePreviewSheet({ passage, questions, onClose }) {
	...
	const QNum = ({ n }) => (
		<span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-sky-600 text-white text-xs font-bold mr-2 shrink-0">
			{n}
		</span>
	);
								...
								>
									<span
										className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold shrink-0 ${
											correct ? "border-emerald-500 bg-emerald-500 text-white"
											: wrong ? "border-red-400 bg-red-100 text-red-700"
											: sel ? "border-sky-500 bg-sky-500 text-white"
											: "border-outline/50 text-on-muted"
										}`}
									>
										{correct ?
	...
	return (
		<div className="fixed inset-0 z-[70] bg-gray-50 flex flex-col overflow-hidden">
			<div className="h-14 flex items-center px-5 border-b border-outline/30 gap-3 bg-white shrink-0">
				...
				<span className="px-2.5 py-0.5 text-xs font-bold bg-sky-100 text-sky-700 rounded-full">
					Xem trước
				</span>
				<div className="flex-1" />
				{submitted && score && score.total > 0 && (
				...
				<div className="w-1/2 border-r border-outline/30 overflow-y-auto p-6 bg-white space-y-4">
					...
					{passage.image_url && (
						<img
							src={passage.image_url}
							alt={passage.title || ""}
							className="w-full rounded-xl object-contain max-h-48 bg-surface-low border border-outline"
						/>
					)}
					{passage.audio_url && (
					...
					{!passage.audio_url && passage.transcript && (
						<p className="text-sm text-charcoal leading-loose whitespace-pre-wrap bg-surface-low rounded-xl p-4">
							{passage.transcript}
						</p>
					)}
					{passage.description && <p className="text-sm text-on-muted">{passage.description}</p>}
...
function ListeningPassageEditorSheet({
	passageId,
	initialPassage,
	onClose,
	onSaved,
	...
	const [form, setForm] = useState(
		initialPassage ?
			{
				title: initialPassage.title || "",
				audio_url: initialPassage.audio_url || "",
				transcript: initialPassage.transcript || "",
				description: initialPassage.description || "",
				level: initialPassage.level || "",
				topic: initialPassage.topic || "",
				source: initialPassage.source || "",
				duration_sec: initialPassage.duration_sec || "",
				image_url: initialPassage.image_url || "",
			}
	...
	const handleAudioSelect = async (file) => {
		...
		if (!AUDIO_TYPES.includes(file.type))
			return setAlert({ type: "error", msg: "Chỉ chấp nhận file âm thanh/video." });
		if (file.size > MAX_AUDIO_MB * 1024 * 1024)
			return setAlert({ type: "error", msg: `File tối đa ${MAX_AUDIO_MB}MB.` });
		setUploading(true);
		try {
			const fd = new FormData();
			fd.append("audio", file);
			const r = await api.post("/admin/listening-passages/upload", fd, {
				headers: { "Content-Type": "multipart/form-data" },
			});
			setForm((prev) => ({ ...prev, audio_url: r.data.url }));
		} catch {
			setAlert({ type: "error", msg: "Không thể tải file âm thanh lên." });
		} finally {
			setUploading(false);
	...
	const handleImageSelect = async (file) => {
		...
		if (file.size > 5 * 1024 * 1024)
			return setAlert({ type: "error", msg: "File ảnh tối đa 5MB." });
	...
	const handleSavePassage = async () => {
		...
		try {
			...
			setAlert({
				type: "success",
				msg: currentId ? "Đã cập nhật bài nghe." : "Đã thêm bài nghe mới.",
			});
			onSaved();
		} catch (e) {
			setAlert({ type: "error", msg: e.message });
	...
	const handleUnlinkQuestion = async (q) => {
		if (!(await confirm(`Bỏ liên kết câu hỏi này khỏi bài nghe? Câu hỏi vẫn còn trong ngân hàng.`)))
			return;
		try {
	...
	const handleImportSelected = async () => {
		...
		try {
			await Promise.all(
				ids.map((id) =>
					api.put(`${apiBase}/question-bank/${id}`, {
						listening_passage_id: currentId,
						passage_id: null,
					}),
				),
			);
	...
	const listeningPassageForForm =
		currentId ?
			[
				{
					id: currentId,
					title: form.title || "(Bài nghe hiện tại)",
					level: form.level,
					question_count: questions.length,
				},
			]
		...
		<>
			<div className="fixed inset-0 z-50 bg-white flex flex-col overflow-hidden">
				...
				<div className="h-14 flex items-center px-4 border-b border-outline/30 gap-3 bg-white shrink-0">
					...
					<div className="flex-1 min-w-0">
						<h2 className="font-bold text-sm truncate">
							{form.title || (currentId ? "Bài nghe" : "Bài nghe mới")}
						</h2>
						{currentId && <p className="text-xs text-on-muted">{questions.length} câu hỏi</p>}
					</div>
					<button
						onClick={() => setPreview(true)}
						disabled={!currentId && !form.audio_url}
						className="flex items-center gap-1.5 px-3 py-2 border border-outline rounded-xl text-sm font-medium hover:bg-surface-low disabled:opacity-40 transition-colors"
					>
						<span className="material-symbols-outlined text-[18px]">visibility</span>
						Xem trước
							...
							{form.audio_url ?
								<div className="space-y-2">
									...
									<div className="flex gap-2">
										...
										>
											<span className="material-symbols-outlined text-[14px]">swap_horiz</span>Đổi
											file
										</button>
										<button
											type="button"
								...
								>
									...
									:	<>
											...
											<p className="text-xs font-semibold text-charcoal">
												Nhấn hoặc kéo thả file âm thanh
											</p>
											<p className="text-xs text-on-muted">
												MP3, WAV, OGG, AAC, M4A — tối đa {MAX_AUDIO_MB}MB
											</p>
										</>
									}
								</div>
                  ...
                  ${imgUploading ? "border-sky-400/50 bg-sky-50" : "border-outline hover:border-sky-400 hover:bg-sky-50/30"}`}
								>
									{imgUploading ?
										<div className="flex flex-col items-center gap-1.5">
											...
											<p className="text-xs text-sky-600 font-semibold">Đang tải...</p>
										</div>
									:	<>
											<span className="material-symbols-outlined text-3xl text-on-muted/40 block mb-1">
												add_photo_alternate
						...
						<div>
							...
							<textarea
								...
								rows={2}
								placeholder="Tóm tắt nội dung bài nghe..."
									...
									return (
										...
										>
											<div className="flex items-start gap-3">
												...
												<div className="flex-1 min-w-0">
													<p className="text-sm font-medium text-charcoal line-clamp-2">
														<span className="text-sky-600 font-bold mr-1">{qi + 1}.</span>
														{q.question_text}
													</p>
													{Array.isArray(q.options) && q.options.length > 0 && (
														<div className="mt-1.5 space-y-0.5">
												...
												<div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
													...
													<button
														onClick={() => handleUnlinkQuestion(q)}
														title="Bỏ liên kết khỏi bài nghe này"
														className="p-1.5 rounded-lg text-on-muted hover:bg-amber-50 hover:text-amber-600"
													>
														<span className="material-symbols-outlined text-[16px]">link_off</span>
													</button>
					...
					>
						...
						<div className="overflow-y-auto p-6">
							<QuestionForm
								...
								setForm={setQForm}
								passages={[]}
								listeningPassages={listeningPassageForForm}
							/>
						</div>
						<div className="flex justify-end gap-3 px-6 py-4 border-t border-outline/20">
							<Button
					...
					>
						<div className="flex items-center justify-between px-6 py-4 border-b border-outline/30 shrink-0">
							<div>
								<h3 className="font-bold text-charcoal">Nhập câu hỏi từ ngân hàng</h3>
								<p className="text-xs text-on-muted mt-0.5">
									Chọn câu hỏi để liên kết vào bài nghe này
								</p>
							</div>
							<button
								onClick={() => setImportModal(false)}
										...
										>
											...
											<div className="flex-1 min-w-0">
												...
												<div className="flex items-center gap-1.5 mt-1">
													...
													{q.listening_passage_id && (
														<span className="text-[10px] px-1.5 py-0.5 bg-sky-50 border border-sky-200 rounded font-semibold text-sky-700">
															Bài nghe khác
														</span>
													)}
													{q.passage_id && (
...
function ListeningPassagesTab({ passages, onRefresh, setAlert }) {
	...
	const handleDelete = async (p) => {
		if (
			!(await confirm(
				`Xóa bài nghe "${p.title || "này"}"? Các câu hỏi liên kết sẽ bị hủy liên kết.`,
			))
		)
			return;
		try {
			await api.delete(`/admin/listening-passages/${p.id}`);
			setAlert({ type: "success", msg: "Đã xóa bài nghe." });
			onRefresh();
		} catch (e) {
			setAlert({ type: "error", msg: e.message });
		}
	...
	const handleTranscribe = async (p) => {
		...
		try {
			...
			if (viewPassage && viewPassage.id === p.id) {
				setViewPassage((prev) => ({
					...prev,
					transcript_segments: r.data.segments,
			...
			{passages.length === 0 ?
				<div className="glass-card rounded-2xl py-20 text-center">
					...
					<p className="text-sm text-on-muted mt-1">
						Nhấn "Thêm bài nghe" để tải lên file âm thanh đầu tiên.
					</p>
				</div>
			:	<div className="grid gap-4">
					{passages.map((p) => (
							...
							<div className="p-5">
								<div className="flex items-start justify-between gap-3">
									<div className="flex-1 min-w-0">
										...
										{p.source && <p className="text-xs text-on-muted mb-2">Nguồn: {p.source}</p>}
										{p.description && (
											<p className="text-sm text-on-muted line-clamp-1">{p.description}</p>
										)}
										{p.audio_url && (
											<audio
												controls
							...
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<div className="flex flex-wrap gap-2 mt-0.5">
										...
										{fmtDuration(viewPassage.duration_sec) && (
											<span className="text-xs text-on-muted">
												{fmtDuration(viewPassage.duration_sec)}
											</span>
										)}
                    ...
                    disabled:opacity-50`}
						...
						<div className="overflow-y-auto p-6 space-y-4">
							{viewPassage.image_url && (
								<img
									src={viewPassage.image_url}
									alt={viewPassage.title || ""}
									className="w-full rounded-xl max-h-48 object-contain bg-surface-low border border-outline"
								/>
							)}
...
function AIPreviewCard({ q, index, selected, onToggle }) {
	...
	return (
		<div
			className={`rounded-xl border-2 transition-all ${selected ? "border-sumire-purple bg-sumire-purple/5" : "border-outline/30 bg-white opacity-60"}`}
		>
			<div className="flex items-start gap-3 p-3">
				<input
					...
					checked={selected}
					onChange={onToggle}
					className="mt-0.5 w-4 h-4 accent-sumire-purple shrink-0"
				/>
				<div className="flex-1 min-w-0">
					<div className="flex flex-wrap gap-1 mb-1">
						{typeCfg && (
							<span
								className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold border ${typeCfg.color}`}
							>
				...
				<button
					onClick={() => setOpen((o) => !o)}
					className="p-1 text-on-muted hover:text-charcoal transition-colors shrink-0"
				>
				...
				<div className="px-4 pb-3 space-y-2 border-t border-outline/20 pt-2">
							...
							<div className="flex flex-wrap gap-1">
								{q.options.map((opt, i) => {
									...
									return (
										<span
											key={i}
											className={`text-xs px-2 py-0.5 rounded-full border ${isCorrect ? "border-emerald-400 bg-emerald-50 text-emerald-700 font-bold" : "border-outline/40 text-on-muted"}`}
										>
											{String.fromCharCode(65 + i)}. {opt}
										</span>
									);
					...
					{q.question_type === "matching" && Array.isArray(q.options) && (
						<div className="flex flex-wrap gap-1.5">
							{q.options.map((p, i) => (
								<span
									key={i}
									className="text-xs px-2 py-0.5 rounded-full bg-amber-50 border border-amber-200 text-amber-800"
								>
									{p.left} → {p.right}
								</span>
							))}
					...
					{q.question_type === "ordering" && Array.isArray(ca) && (
						<div className="flex flex-wrap gap-1">
							{ca.map((item, i) => (
								<span
									key={i}
									className="text-xs px-2 py-0.5 rounded-full bg-orange-50 border border-orange-200 text-orange-800"
								>
									{i + 1}. {item}
								</span>
							))}
					...
					{(q.question_type === "fill_blank" || q.question_type === "short_answer") && ca && (
						<p className="text-xs text-emerald-700">
							Đáp án: <strong>{Array.isArray(ca) ? ca.join(", ") : ca}</strong>
						</p>
					)}
...
function AIGenerateModal({
	...
	const toggleType = (t) =>
		setC(
			"question_types",
			config.question_types.includes(t) ?
				config.question_types.filter((x) => x !== t)
			:	[...config.question_types, t],
		);
	...
	const generate = async () => {
		...
		if (config.source === "listening" && !config.listening_passage_id)
			return setError("Chọn bài nghe.");
		if (config.source === "custom" && !config.custom_content.trim())
			return setError("Nhập nội dung.");
		setStep("loading");
		setError("");
		try {
			const body = {
			...
			else if (config.source === "listening")
				body.listening_passage_id = config.listening_passage_id;
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-on-muted">
								...
								{step === "results" &&
									`${results.length} câu hỏi được tạo — chọn để lưu vào ngân hàng`}
							</p>
						</div>
					</div>
					<button
						onClick={handleClose}
						className="text-on-muted hover:text-charcoal transition-colors"
					>
				...
				<div className="flex-1 overflow-y-auto">
					...
					{(step === "config" || step === "loading") && (
						<div className="p-5 space-y-5">
							...
							{/* Source */}
							<div>
								<label className="block text-xs font-bold text-on-muted uppercase tracking-wide mb-2">
									Nguồn nội dung
								</label>
									...
									{[
										...
										<button
											...
											className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border-2 text-sm font-semibold flex-1 justify-center transition-all ${
												...
												:	"border-outline/40 text-on-muted hover:border-sumire-purple/40"
											}`}
										>
								...
								{config.source === "passage" && (
									<select
										value={config.passage_id}
										onChange={(e) => setC("passage_id", e.target.value)}
								...
								{config.source === "listening" && (
									<select
										value={config.listening_passage_id}
										onChange={(e) => setC("listening_passage_id", e.target.value)}
										className="w-full px-4 py-2.5 bg-white border border-outline rounded-xl text-sm outline-none focus:border-sumire-purple transition-colors"
									>
								...
								{config.source === "custom" && (
									<textarea
										value={config.custom_content}
										onChange={(e) => setC("custom_content", e.target.value)}
										rows={5}
										placeholder="Nhập đoạn văn/transcript tiếng Nhật hoặc chủ đề muốn tạo câu hỏi..."
										...
										return (
											<button
												...
												className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border-2 text-xs font-semibold transition-all ${
													active ?
														`${t.color} shadow-sm`
													:	"border-outline/40 text-on-muted bg-white hover:border-outline"
												}`}
											>
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
								<div>
									<label className="block text-xs font-semibold text-on-muted mb-1">
										Số câu hỏi
									</label>
									<select
										value={config.count}
								...
								<div>
									...
									<select
										value={config.level}
										onChange={(e) => setC("level", e.target.value)}
										className="w-full px-3 py-2.5 bg-white border border-outline rounded-xl text-sm outline-none focus:border-sumire-purple transition-colors"
									>
								...
								<div>
									...
									<select
										...
										onChange={(e) => setC("difficulty", e.target.value)}
										className="w-full px-3 py-2.5 bg-white border border-outline rounded-xl text-sm outline-none focus:border-sumire-purple transition-colors"
									>
								...
								<div>
									...
									<select
										value={config.skill}
										onChange={(e) => setC("skill", e.target.value)}
										className="w-full px-3 py-2.5 bg-white border border-outline rounded-xl text-sm outline-none focus:border-sumire-purple transition-colors"
									>
							...
							<div>
								...
								<input
									type="text"
									value={config.topic}
									onChange={(e) => setC("topic", e.target.value)}
					...
					{step === "loading" && (
						<div className="flex flex-col items-center justify-center py-16 gap-4 text-sumire-purple">
							<div className="relative">
								...
								<span className="material-symbols-outlined text-2xl absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
									auto_awesome
								</span>
							</div>
							<div className="text-center">
								...
								<p className="text-sm text-on-muted mt-1">
									AI đang phân tích nội dung và soạn câu hỏi
								</p>
							</div>
						</div>
					)}
					...
					{step === "results" && (
						<div className="p-5">
							...
							<div className="flex items-center justify-between mb-3">
								<p className="text-sm text-on-muted">
									{selectedCount}/{results.length} câu được chọn
								</p>
								<div className="flex gap-2 text-xs">
									<button
										...
										className="text-sumire-purple hover:underline"
									>
										Chọn tất cả
									</button>
									...
									<button
										onClick={() =>
											setSelected(Object.fromEntries(results.map((_, i) => [i, false])))
										}
										className="text-on-muted hover:text-charcoal hover:underline"
									>
										Bỏ chọn
									</button>
								</div>
				...
				<div className="p-5 border-t border-outline/30 flex justify-between items-center shrink-0 gap-3">
					{step === "config" && (
						<>
							<button
								onClick={handleClose}
								className="px-4 py-2 rounded-xl border border-outline text-sm text-on-muted hover:bg-surface-low transition-colors"
							>
								Hủy
							</button>
							<button
					...
					{step === "loading" && (
						<div className="flex-1 text-center text-sm text-on-muted">
					...
					{step === "results" && (
						<>
							<button
								...
								className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-outline text-sm text-on-muted hover:bg-surface-low transition-colors"
							>
...
function GlobalBankTab({ apiBase, setAlert, onImported }) {
	...
	useEffect(() => {
		load(page, search, fLevel, fType);
	}, [page, fLevel, fType, load]);
	...
	const doImport = async () => {
		...
		setImporting(true);
		try {
			const r = await api.post(`${apiBase}/question-bank/import-from-global`, {
			...
			setAlert({ type: "error", msg: e.response?.data?.error || e.message });
		} finally {
			setImporting(false);
		}
	};
			...
			<div className="glass-card rounded-2xl p-5 mb-4">
				<div className="flex flex-wrap gap-3 items-center">
					<div className="flex-1 min-w-[220px] relative">
						<span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-muted text-lg">
							search
						</span>
						<input
							value={search}
							onChange={(e) => onSearch(e.target.value)}
							placeholder="Tìm câu hỏi trong ngân hàng chung..."
			...
			<div className="flex items-center justify-between mb-3">
				<p className="text-sm text-on-muted">
					{selectedIds.length > 0 ?
						<>
							<strong className="text-charcoal">{selectedIds.length}</strong> câu được chọn
						</>
					:	"Chọn câu hỏi để thêm vào ngân hàng của bạn"}
				</p>
				<Button
					onClick={doImport}
					loading={importing}
			...
			<div className="glass-card rounded-2xl overflow-hidden mb-6">
					...
					<table className="w-full border-collapse text-left">
						<thead>
							<tr className="bg-surface-low border-b border-outline/30">
								...
								<th className="px-5 py-3 text-xs font-bold text-on-muted uppercase tracking-wide">
									Câu hỏi
								</th>
								<th className="px-4 py-3 text-xs font-bold text-on-muted uppercase tracking-wide">
									Loại
								</th>
								<th className="px-4 py-3 text-xs font-bold text-on-muted uppercase tracking-wide w-16">
									Cấp độ
								</th>
								<th className="px-4 py-3 text-xs font-bold text-on-muted uppercase tracking-wide">
									Độ khó
								</th>
								<th className="px-4 py-3 text-xs font-bold text-on-muted uppercase tracking-wide text-right">
									Xem
								</th>
							</tr>
						</thead>
						<tbody className="divide-y divide-outline/20">
								...
								Array.from({ length: 6 }).map((_, i) => (
									<tr key={i}>
										{Array.from({ length: 6 }).map((_, j) => (
											...
											>
												<div
													className="h-4 bg-surface-low rounded animate-pulse"
													style={{ width: j === 1 ? "80%" : "50%" }}
												/>
											</td>
										))}
									</tr>
							...
							:	items.map((item) => {
									...
									return (
										<tr
											key={item.id}
											className={`group transition-colors ${checked ? "bg-tsubaki-red/5" : "hover:bg-surface-low/60"}`}
										>
												...
												<div className="flex flex-wrap gap-1.5 mt-1.5">
													{item.reading_passages && (
														<span className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 bg-amber-50 border border-amber-200 rounded font-semibold text-amber-700">
															...
															{item.reading_passages.title || "Bài đọc"}
														</span>
													)}
													{item.topic && (
											...
											<td className="px-4 py-4">
												<TypeBadge
													type={item.question_type || "single_choice"}
													sm
												/>
											</td>
											<td className="px-4 py-4">
				...
				<div className="px-5 py-3 bg-surface-low/50 border-t border-outline/20 flex justify-between items-center">
					<p className="text-xs text-on-muted">
						{loading ?
							"..."
						:	`Hiển thị ${total === 0 ? 0 : Math.min((page - 1) * LIMIT + 1, total)}–${Math.min(page * LIMIT, total)} / ${total} câu hỏi`
						}
					</p>
					<div className="flex gap-1">
						<button
...
function QuestionBankManager({
	...
	const fetchStats = useCallback(async () => {
		try {
			const r = await api.get(`${apiBase}/question-bank/stats`);
			setStats(r.data);
		} catch (_) {}
	}, [apiBase]);
	...
	const fetchPassages = useCallback(async () => {
		try {
			const r = await api.get(`${apiBase}/reading-passages`);
			setPassages(r.data || []);
		} catch (_) {}
	}, [apiBase]);
	...
	const fetchListeningPassages = useCallback(async () => {
		try {
			const r = await api.get("/admin/listening-passages");
			setListeningPassages(r.data || []);
		} catch (_) {}
	}, []);

	const fetchItems = useCallback(
		async (p, l, sk, d, st, tp, pid, lpid, s) => {
			setLoading(true);
			try {
				const params = new URLSearchParams({ page: p, limit: LIMIT });
				if (l) params.set("level", l);
	...
	const handleSave = async () => {
		if (!form.question_text.trim())
			return setAlert({ type: "error", msg: "Nội dung câu hỏi là bắt buộc." });
		setSaving(true);
		try {
	...
	const handleDelete = async (row) => {
		...
		try {
			await api.delete(`${apiBase}/question-bank/${row.id}`);
	...
	const handleApprove = async (row) => {
		try {
			await api.put(`${apiBase}/question-bank/${row.id}`, { status: "approved" });
			setAlert({ type: "success", msg: "Đã duyệt." });
			refresh();
		} catch (e) {
			setAlert({ type: "error", msg: e.message });
		}
		...
		<Layout title={title}>
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
				...
				<StatCard
					icon="headphones"
					label="Bài nghe"
					value={listeningPassages.length}
			...
			<div className="flex gap-1 mb-5 border-b border-outline/30">
				{[
					...
					...(showGlobalImport ?
						[{ key: "global", label: "Ngân hàng chung", icon: "public" }]
					:	[]),
				].map((tab) => (
					<button
						key={tab.key}
              ...
              ${activeTab === tab.key ? "border-tsubaki-red text-tsubaki-red" : "border-transparent text-on-muted hover:text-charcoal"}`}
					>
						...
						{tab.key === "passages" && passages.length > 0 && (
							<span className="w-5 h-5 flex items-center justify-center rounded-full bg-amber-100 text-amber-700 text-[10px] font-bold">
								{passages.length}
							</span>
						)}
						{tab.key === "listening" && listeningPassages.length > 0 && (
							<span className="w-5 h-5 flex items-center justify-center rounded-full bg-sky-100 text-sky-700 text-[10px] font-bold">
								{listeningPassages.length}
							</span>
						)}
					</button>
				))}
			...
			{activeTab === "questions" && (
				<>
						...
						<div className="flex flex-wrap gap-3 items-center mb-4">
							...
							{hasFilters && (
								<button
									...
									className="flex items-center gap-1 px-3 py-2.5 rounded-xl text-sm text-tsubaki-red border border-tsubaki-red/30 hover:bg-tsubaki-red/5 transition-colors"
								>
									<span className="material-symbols-outlined text-[16px]">filter_list_off</span>Xóa
									bộ lọc
								</button>
							)}
						</div>
						<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
							{[
								...
								},
								{
									...
									opts: [["", "Tất cả"], ...LEVELS.map((l) => [l, l])],
								},
								{
									label: "Kỹ năng",
									value: filterSkill,
									onChange: setFilterSkill,
									opts: [["", "Tất cả"], ...SKILLS.map((s) => [s, s])],
								},
								{
									label: "Độ khó",
									value: filterDiff,
									onChange: setFilterDiff,
									opts: [["", "Tất cả"], ...DIFFICULTIES.map((d) => [d.value, d.label])],
								},
								{
									label: "Trạng thái",
									value: filterStatus,
									onChange: setFilterStatus,
									opts: [["", "Tất cả"], ...STATUSES.map((s) => [s.value, s.label])],
								},
									...
									: items.length === 0 ?
										<tr>
											...
											>
												...
												<p className="text-sm text-on-muted mt-1">
													{hasFilters ? "Thử thay đổi bộ lọc." : 'Nhấn "Thêm câu hỏi" để bắt đầu.'}
												</p>
											</td>
										</tr>
									:	items.map((item) => {
											...
											return (
												<tr
													key={item.id}
													className="group hover:bg-surface-low/60 transition-colors"
												>
													<td className="px-5 py-4 max-w-xs">
														<p className="text-sm font-medium text-charcoal line-clamp-2 leading-snug">
															{item.question_text}
					...
					<button
						onClick={() => setAiModal(true)}
						className="w-full text-left glass-card rounded-2xl p-5 border-l-4 border-l-sumire-purple relative overflow-hidden hover:shadow-lg transition-shadow group"
					>
						<div className="absolute -right-4 -top-4 opacity-[0.06] pointer-events-none group-hover:opacity-[0.10] transition-opacity">
							<span className="material-symbols-outlined text-[120px] text-sumire-purple">
								auto_awesome
							</span>
						</div>
						<div className="flex items-start gap-4 relative">
							...
							<div className="flex-1">
								...
								<p className="text-sm text-on-muted max-w-2xl">
									Ngân hàng hiện có <strong className="text-charcoal">{stats.total}</strong> câu hỏi
									và <strong className="text-charcoal">{passages.length}</strong> bài đọc.
									{stats.pending > 0 && (
										<>
											{" "}
											Còn <strong className="text-amber-600">{stats.pending} câu chờ duyệt</strong>.
										</>
									)}{" "}
									Nhấp vào đây để tạo hàng loạt câu hỏi từ bài đọc hoặc nội dung bất kỳ.
								</p>
							</div>
							<span className="material-symbols-outlined text-sumire-purple/50 group-hover:text-sumire-purple transition-colors text-2xl shrink-0">
								arrow_forward
							</span>
						</div>
					</button>
